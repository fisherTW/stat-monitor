...
class Auth_OpenID_Consumer {
	...
	//	var_dump($m);
        ...
        $endpoint = $disco->getNextService($this->discoverMethod,
                                           $this->consumer->fetcher);//var_dump($endpoint);die;
        ...
        if ($endpoint === null) {
			//file_put_contents("log.txt", "錯誤的OpenID(URL) 或 discover fail: no OP server endpoint，檢查XRDS檔案的設置路徑\n", FILE_APPEND);
			$error = "錯誤的OpenID(URL) 或 discover fail: no OP server endpoint，檢查XRDS檔案的設置路徑";
    ...
    function beginWithoutDiscovery($endpoint, $anonymous=false)
    {
        ...
        $this->session->set($this->_token_key,
              $loader->toSession($auth_req->endpoint));//file_put_contents("log.txt", serialize($auth_req), FILE_APPEND);
...
class Auth_OpenID_GenericConsumer {
	...
	//	var_dump("check id _idResCheckForFields: ");
	//	var_dump($result);
	...
	//var_dump($message);die;
        ...
        $endpoint = $result;
	//	var_dump("check _verifyDiscoveryResults OK!!");
	//	var_dump($endpoint);
    ...
    function _idResCheckSignature($message, $server_url)
    {
		...
		//file_put_contents("log.txt", "association: ", FILE_APPEND);
       // file_put_contents("log.txt", $assoc_handle . "\n", FILE_APPEND);
...
class Auth_OpenID_AuthRequest {
    ...
    function Auth_OpenID_AuthRequest($endpoint, $assoc)
    {
        ...
        $this->message = new Auth_OpenID_Message(
            $endpoint->preferredNamespace());   // problem here
