...
class Auth_Yadis_Manager {
    ...
    {
	//	var_dump("now service: ");
	//	var_dump($this->services);
        if ($this->services) {
            $this->_current = array_shift($this->services);//var_dump("next service: ");
     ...
     */
    function forURL($url)
    {//var_dump($this->starting_url . ",,,," . $this->yadis_url . ",,,,," . $url);die;
...
class Auth_Yadis_Discovery {
    ...
    {//var_dump($discover_cb); Auth_OpenID_discover
        $manager = $this->getManager();
		var_dump("pre: manager");
	//var_dump($manager);
        if (!$manager || (!$manager->services)) {//var_dump("111");
            $this->destroyManager();
            ...
            $manager = $this->createManager($services, $yadis_url);var_dump("session no endpoint and create one: ");var_dump($services);//die;
        }
	
        if ($manager) {
            $loader = new Auth_Yadis_ManagerLoader();
            $service = $manager->nextService();	//var_dump("session has manager: ");var_dump($service);//die;
            $this->session->set($this->session_key,
                                serialize($loader->toSession($manager)));
        } else {
            $service = null;
        }
//var_dump("return service: ");var_dump($service);//die;
        return $service;
    }
    ...
    function getManager($force=false)
    {
        ...
        $manager_str = $this->session->get($this->getSessionKey());//var_dump($manager_str);die;
        $manager = null;
        if ($manager_str !== null) {
            $loader = new Auth_Yadis_ManagerLoader();
        ...
        if ($manager && ($manager->forURL($this->url) || $force)) {//var_dump($manager);die;
            return $manager;
        }
    }
